...
rotations['b'] = RY(angle).dot(RZ(+angle).dot(RX(+angle)))
rotations['c'] = RY(angle).dot(RZ(-angle).dot(RX(-angle)))
vectors['a'] = np.array([-sq2, sq2, sq2])
vectors['b'] = np.array([+sq2, sq2, -sq2])
vectors['c'] = np.array([+2*sq2,-sq2, -sq2])

scales = {'a':0.73, 'b':0.7, 'c':0.6}
...
linden = L_system('A',{'A':'[CaB][bB][cB]','B':'[BaA]','C':'C'})
result = linden.get_iteration(n=12)
